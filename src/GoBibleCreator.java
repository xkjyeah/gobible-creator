...
public abstract class GoBibleCreator 
{
	 ...
	 **/
	public final static String SUB_VERSION = "4.5";
	...
	protected static String baseSourceDirectory = null;
        ...
        // the extension that is used by the USFMr files
        // introduced in version 2.3
        protected static String usfmSourceFileExtension = null;
        // file to help parse USFM
        // introduced in version 2.4.6
        protected static String usfmParseConfigFile = "USFMSettings.txt";
    ...
    public static void main(String args[]) throws IOException
    {
    	  ...
    	  // updateSVNRevisionNumber(); // updates versionString
    ...
    /*
     * 
     * Unfortunately we cannot do this because MIDP
     * specification states that version number is 
     * three dot-separated digits.
     * */
    /*
    private static void updateSVNRevisionNumber() {
    }*/
...
//			}
	...
	public static File getJarDirectory()
	{
		...
		String[] paths = (classPath + pathSeparator + ".").split(pathSeparator);
		...
		System.out.println("Error: couldn't find the path to GoBibleCreator.jar. Assuming resources can be found at . (current directory)");
...
//		attributes.putValue("Manifest-Version", "1.0");
                ...
                {
        		attributes.putValue("MIDlet-Name", (collection.name + " " + applicationName).trim());
                        attributes.putValue("MIDlet-1", (collection.name + " " + applicationName).trim() + ", Icon.png, GoBible");
                }
                else
                {
           		attributes.putValue("MIDlet-Name", (collection.name + NAME_APPENDAGE).trim());
                        attributes.putValue("MIDlet-1", (collection.name + NAME_APPENDAGE).trim() + ", Icon.png, GoBible");
                }
...
+ "\u066b\u06d0\u06d0\u06d0\u06d0\u06d0\u06d0\u06d0\u06d0\u06d0"
	...
	public static void createJadFile(File directory, Collection collection, long jarFileLength, String url) throws IOException
	{
		...
		PrintWriter writer = new PrintWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream(jadFile), "UTF-8")));
                ...
                if (applicationName != null)
                {
        		writer.println("MIDlet-1: " + (collection.name + " " + applicationName).trim() + ", Icon.png, GoBible");
                	writer.println("MIDlet-Name: " + (collection.name + " " + applicationName).trim());
                }
                else
                {
...
class Book
{
	...
	public int endChapter = -1;
        ...
        /* USFM Interpreter */
        private static USFMParse interpreter = null;
	...
	public Book(String baseSourceDirectory, String sFilename, char cSTYLE_RED, String fileCodepage, boolean useRedLettering, String sTitleTag)
	{
            ...
            try
            {
                ...
                USFMLex scanner = null;
                USFMSymbol currentSymbol = USFMSymbol.initial();
                String titleTag = sTitleTag.substring(1);
                
                scanner = new USFMLex(new BufferedReader(new InputStreamReader(new FileInputStream(fIn), fileCodepage)));

                /* Find the tag preceding the book name */
                LOOP: while ( true ) {
                        currentSymbol = scanner.yylex();

                        switch (currentSymbol.type) {
                        case TAG_OPEN:
                            if (currentSymbol.data.equals(titleTag)) {
                                    break LOOP;
                            }
                            break;
                        case EOF:
                            ...
                            return;
                /* TODO: parsing abruptly halts ... could we just try? */
                        default:
                        }
                }
                ...
                for (currentSymbol = scanner.yylex();
                    currentSymbol.type == SymbolType.TEXT;
                    currentSymbol = scanner.yylex()) {
                    
                    bookName.append(currentSymbol.data);
                }
                name = bookName.toString().trim();

                /* extract the next chapter */
                int cx = 0;
                LOOP: for ( ; true ; currentSymbol = scanner.yylex() ) {
                        SKIPLEX: while (true) {
                            switch (currentSymbol.type) {
                            case TAG_OPEN:
                                    if (currentSymbol.data.equals("c")) {
                                        cx++;
                                        // follow the behaviour of the previous code
                                        currentSymbol = scanner.yylex();
                                        if (currentSymbol instanceof NumericTextSymbol &&
                                            ((NumericTextSymbol)currentSymbol).number == 1) {
                                            startChapter = cx;
                                            break LOOP;
                                        }
                                        continue SKIPLEX;
                ...
                Chapter c;
                interpreter.reset(currentSymbol, scanner);

                while ( (c = interpreter.parseChapter()) != null) {
                        chapters.addElement(c);
                }
